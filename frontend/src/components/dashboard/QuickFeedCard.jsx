...
const QuickFeedCard = ({ portionTarget }) => {
  const targetValue = typeof portionTarget === "number" ? `${portionTarget.toFixed(0)} g` : "50 g"
  return (
    <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-6 shadow-sm flex flex-col items-center justify-center text-center cursor-pointer hover:bg-emerald-100 transition-colors">
      ...
      <p className="text-xs text-emerald-700/70 mt-1">Target portion: {targetValue}</p>
    </div>
  )
}
