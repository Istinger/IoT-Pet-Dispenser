...
const NextFeedCard = ({ dispensing, portionTarget, portionDelivered }) => {
  ...
  const statusLabel = dispensing ? "Dispensing now" : "Idle"

  return (
    <div className="bg-white border border-slate-100 rounded-xl p-6 shadow-sm flex flex-col justify-between">
      <div>
        <h3 className="text-xs uppercase font-bold text-slate-400">Dispenser Status</h3>
        <p className="text-2xl font-bold mt-1">{statusLabel}</p>
        <p className="text-xs text-slate-500 mt-1">Target: {targetValue} · Delivered: {deliveredValue}</p>
      </div>
